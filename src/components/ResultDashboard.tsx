...
export default function ResultDashboard({ responses, onRestart, onGoToMain }: ResultDashboardProps) {
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center">
        <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-100 relative overflow-hidden">
          ...
          <div className="absolute top-0 inset-x-0 h-2 bg-gradient-to-r from-amber-500 via-amber-600 to-amber-700 animate-pulse" />
          
          <div className="flex flex-col items-center justify-center space-y-6 py-12">
            <div className="relative">
              <div className="w-16 h-16 rounded-full border-4 border-slate-100 border-t-amber-600 border-r-amber-700 animate-spin" />
              <Scale className="absolute inset-0 m-auto w-7 h-7 text-amber-600 animate-bounce" />
            </div>
            ...
            <div className="w-full max-w-xs bg-slate-100 h-2 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-amber-500 to-amber-700 animate-pulse" style={{ width: '100%' }} />
            </div>
  ...
  return (
    <div className="relative max-w-3xl mx-auto px-2 sm:px-4 py-4 sm:py-8 md:py-12 space-y-5.5" id="result-dashboard-stage">
      ...
      <div className="absolute bottom-[15%] left-[10%] w-[350px] h-[350px] rounded-full bg-amber-300/[0.05] blur-[90px] pointer-events-none -z-10" />
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-850 to-slate-800 rounded-2xl sm:rounded-3xl p-5 sm:p-8 text-white relative overflow-hidden shadow-xl border border-slate-800 animate-fade-in">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-5">
          <div className="space-y-1.5 flex-1 w-full text-left">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-550/25 text-amber-400 border border-amber-600/30 text-[10px] font-black tracking-wider uppercase">
          ...
          <div className="shrink-0 flex items-center gap-3 bg-white/10 backdrop-blur-md p-3.5 sm:p-4 rounded-xl border border-white/10 w-full md:w-auto shadow-inner">
            <div className="w-11 h-11 rounded-lg bg-amber-600 flex items-center justify-center text-white shrink-0">
              <Scale className="w-5.5 h-5.5" />
            </div>
            <div>
              <span className="text-[9px] text-slate-300 font-bold block leading-none mb-1">나의 진단 자격 등급</span>
              <span className="text-base font-black text-amber-400 leading-none">{est.eligibilityGrade}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-xl border border-slate-100 space-y-6 sm:space-y-8 text-left">
        ...
        <div className="space-y-4">
          <h3 className="text-base sm:text-lg font-black text-slate-800 tracking-tight flex items-center gap-2">
            <CheckCircle className="w-5 h-5 text-amber-600 stroke-[2.5]" />
          ...
          <div className="p-4 sm:p-5 rounded-xl bg-slate-50 border border-slate-100/60 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
            ...
            <div className="col-span-1 md:col-span-8 space-y-4">
              ...
              {/* Reduced Debt Bar */}
              <div className="space-y-1">
                <div className="flex justify-between items-center text-xs font-bold text-amber-750">
                  <span>회생 완료 후 최종 변제액</span>
                  {est.reductionRate > 0 ? (
                    <span className="font-extrabold text-amber-800 text-right">약 {est.reducedDebtStr} (이자 전액 탕감)</span>
                  ) : est.isFullRepayment ? (
                    <span className="font-extrabold text-amber-800 text-right">약 {est.reducedDebtStr} (장래이자는 100% 탕감)</span>
                  ) : (
              ...
              {est.reductionRate > 0 ? (
                <div className="p-3 bg-amber-100/35 rounded-xl border border-amber-500/10 flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-[11px] sm:text-xs">
                  <span className="font-black text-amber-950 flex shadow-3xs items-center gap-1.5 shrink-0 px-2.5 py-0.5 rounded-lg bg-amber-500/10">법적으로 즉시 탕감 소멸되는 빚:</span>
                  <span className="font-black text-amber-700">총 약 {est.savingsDebtStr} (탕감 완료)</span>
                </div>
              ) : est.isFullRepayment ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 sm:p-5 rounded-2xl border border-slate-100 bg-gradient-to-b from-white to-slate-50/50 space-y-3 flex flex-col justify-between">
            {est.isFullRepayment ? (
              <div className="space-y-3 font-sans w-full">
                ...
                {repaymentMethodTab === 'shorter' ? (
                  <div className="space-y-1.5 animate-fade-in text-left">
                    <span className="text-[10px] sm:text-[11px] font-black text-amber-600 uppercase tracking-widest block">
                      방법 1: 소득 기준 집중 변제형
                    </span>
                    <p className="text-xl sm:text-2xl font-black text-slate-900 leading-none">
                      월 {est.method1MonthlyPaymentStr}
                    </p>
                    <span className="text-xs text-amber-700 font-extrabold bg-amber-50 px-2 py-0.5 rounded border border-amber-100 inline-block">
                      총 {est.method1Term}개월 상환 (조기 종결)
                    </span>
                    <p className="text-xs font-bold text-amber-600 leading-relaxed pt-1">
                  ...
                  <div className="space-y-1.5 animate-fade-in text-left">
                    ...
                    <span className="text-xs text-amber-800 font-extrabold bg-amber-100/60 px-2 py-0.5 rounded border border-amber-200 inline-block">
                      총 36개월 분기 상환 (지출 완화)
                    </span>
                    <p className="text-xs font-bold text-amber-600 leading-relaxed pt-1">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs font-black text-slate-600 leading-relaxed pt-2">
                  (2026년 최저생계비 적용 기준)
                </p>
              </div>
            )}
          ...
          <div className="p-4 sm:p-5 rounded-2xl border border-amber-200 bg-amber-50/20 space-y-2 flex flex-col justify-between">
            <span className="text-[11.5px] sm:text-[12.6px] font-black text-amber-800 uppercase tracking-widest block">법무사 여환동의 안심 케어</span>
            ...
            <div className="pt-2 border-t border-amber-200/50 text-[11.5px] sm:text-[12.6px] font-black text-amber-800 uppercase tracking-widest">
              ★ 기각 시 수임료 100% 환불제 적용 ★
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3.5 justify-center items-center pt-4">
        <button
          onClick={onRestart}
          className="w-full sm:w-auto px-10 py-4 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-700 hover:to-amber-600 text-white font-extrabold text-xs sm:text-sm rounded-2xl shadow-md transition-all flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
        >
