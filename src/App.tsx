...
export default function App() {
  ...
  const handleWorryChipClick = (index: number) => {
    const faqIdMap: Record<number, number> = {
      0: 2, // 전부 최근 대출이에요. -> 최근 1년 이내 대출 (id 2)
      1: 4, // 전화 독촉 스트레스! -> 채무 독촉 언제 중단 (id 4)
      2: 5, // 코인 및 주식 투자손실 -> 주식 코인 투자 (id 5)
      3: 6  // 배우자 몰래 진행하기! -> 직장 가족 몰래 (id 6)
    };
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F5] flex flex-col font-sans selection:bg-amber-200 selection:text-slate-900 relative overflow-hidden" id="main-landing-wrap">
      {/* Beautiful Royal Gold Mesh Wireframe Grid Backdrop Overlay */}
      <div className="absolute inset-0 gold-mesh-grid pointer-events-none -z-20" />
      
      {/* Exquisite Soft Floating Spot Light Auroras under the layers */}
      <div className="absolute top-[8%] left-[10%] w-[380px] h-[380px] rounded-full bg-amber-400/[0.04] blur-[100px] pointer-events-none -z-30 animate-slow-drift-1" />
      <div className="absolute top-[38%] right-[8%] w-[420px] h-[420px] rounded-full bg-emerald-500/[0.03] blur-[120px] pointer-events-none -z-30 animate-slow-drift-2" />
      <div className="absolute bottom-[20%] left-[5%] w-[400px] h-[400px] rounded-full bg-indigo-500/[0.03] blur-[110px] pointer-events-none -z-30" />

      {/* Universal Sticky Header (Fixed) */}
      <Header
      ...
      >
        ...
        <AnimatePresence>
          {consultationOpen && (
            ...
            >
              {/* Header */}
              <div className="bg-slate-900 border-b border-amber-500/10 p-4 font-sans text-white flex items-center justify-between">
              ...
              {submitSuccess ? (
                <div className="p-6 text-center space-y-4 font-sans">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500 font-semibold leading-relaxed">
                      지정하신 희망 일시에 여환동 법무사가 실시간으로 직접 전화를 드려 상세한 자격 진단과 혜택을 설계해 드립니다. 감사합니다.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <form onSubmit={handleReservationSubmit} className="p-4 sm:p-5 space-y-4 font-sans">
                  {/* Tab Selector */}
        ...
        >
          <div className="relative">
            <MessageSquare className={`w-5 h-5 stroke-[2.3] group-hover:rotate-12 transition-transform duration-300 ${
              isOverFooter ? 'text-amber-950' : 'text-amber-400'
            }`} />
            <span className="absolute -top-1 -right-1 flex h-2 w-2">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                isOverFooter ? 'bg-amber-800' : 'bg-amber-400'
              }`}></span>
              <span className={`relative inline-flex rounded-full h-2 w-2 ${
                isOverFooter ? 'bg-amber-900' : 'bg-amber-500'
              }`}></span>
            </span>
          </div>
